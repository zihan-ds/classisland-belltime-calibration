...
| 本版本更新 | **[更新说明-v1.0.2.md](更新说明-v1.0.2.md)**（全部功能更新、设置项、验证方式） |
...
| 大误差复核（变更 > 阈值拒写 **并弹出人工复核提醒**） | 防止校铃钟被人工大规模校准/时钟跳变时静默改掉几秒；实机出现过 5.8 s 的偏差被静默拦下、用户毫不知情 |
| 中位数估计替代趋势外推 | 从 ±1.5 s 的噪声里估斜率并外推，会让偏移在边界间震荡 ±1.5 s |
| 放学校铃布防回退 | 当天最后一节之后内核 `OnBreakingTimeLeftTime` 恒为 0，导致放学边界从不布防 |

### 大误差人工复核提醒

一次要写入的时间偏移与当前偏移相差超过**大误差复核阈值**（设置项，默认 ±3 s）时，插件**拒绝写入**，
并在 ClassIsland 界面上弹出提醒，写明边界时刻、当前偏移、本次测得的所需偏移与差值，要求人工确认后再决定是否手动校准。

- 提醒只在**真的拒写**时出现；**没有时间冷却**——同一个大误差每次都提醒（不想被打扰直接用下面的开关）；
- 由设置项**「人工审核提醒」**（默认开）控制：关掉后大误差仍然拒写，但不再弹提醒、只记日志。
  拒写是数据安全底线，提醒只是可选的打扰，所以两者分开；
- 提醒出现在 ClassIsland 的提醒通道里，可在**设置 → 提醒 → 铃声校时复核**单独关闭（也可调整提醒音、置顶等宿主级偏好）；
- 想随时确认提醒能正常弹出，用插件设置页的**「发送测试提醒」**按钮（该测试不受开关约束）；
- 阈值可在插件设置页调整（1~60 s），改完即生效、无需重启。

> 为什么提醒不能只写日志：**被静默拦下的偏差，用户根本不知道要去看日志**。大误差恰恰是最需要人介入的情形
> （校铃钟被人工校过、系统时钟跳变、信号源切换），所以这条路径必须有出口。

### 手动拟合（设置页按钮）

设置页有一张**「手动拟合」**卡片：按下「手动拟合并应用」后，插件读当天已落盘的样本、**只保留有效样本**
（真正写入过偏移的干净测量；大误差拒写与死区带内的存量不计入），用与运行时**完全相同**的估计器
（近 6 小时中位数；趋势外推仅在样本数/跨度/残差三项门槛同时满足时启用）重算一次，并**立即把结果写入
「应用设置 → 时钟 → 时间偏移」**——**无视学习模式与自动应用开关**（这是一次显式的手动动作，按下即写）。

- **为什么排除 `Applied = false` 的样本**：它们有两种来源，都不是干净测量——死区带内（偏移没动的存量，
  不代表当前基准）与大误差拒写（含未观测到切换的退化绝对口径；实机 2026-09-17 留下过 −0.91 / +0.29 两条，
  与当天真实基准 −7 s 相差近 7 秒）。用它们拟合等于把脏值写进设置项。这里不另造启发式，只用样本自带的标记。
- 写入后插件会把当天样本重新装进运行时估计器，避免下一个边界用旧序列把手动结果顶回去；
- 结果文字写在按钮旁边（写入前后的值、有效样本数、中位数、散度、趋势是否启用），日志同样留痕。
...
│  ├─ BellReviewDecision.cs   大误差复核提醒的阈值/开关判定（纯函数，可离线断言）
│  ├─ BellReviewNotifier.cs   大误差复核提醒：文案 + 交给宿主提醒通道派发
│  ├─ ManualFitService.cs     手动拟合：当天有效样本 → 所需偏移（设置页按钮背后的纯逻辑）
...
└─ tools/
   ...
   └─ ReplayTool/             回放工具：对实机 dump 重跑插件完整判定链；fitter 验证估计算法；notify-test 断言复核提醒判定

docs/HANDOFF.md               开发交接记录（含完整调试历程与实测数据）
更新说明-v1.0.2.md             本版本（1.0.2）全部功能更新、设置项对照与验证方式
...
| 大误差复核阈值 | 3 秒 | 一次要改的偏移超过此值时拒写；同时决定是否弹人工复核提醒。改完即生效 |
| 人工审核提醒 | 开 | 关掉后大误差仍然拒写，但不再弹提醒（只记日志） |
...
ReplayTool <dump.wav> <模板目录> <边界在dump内秒数>   # 对实机 dump 重跑完整判定链
ReplayTool fitter <样本序列文件>                     # 复算偏移估计算法
ReplayTool notify-test                              # 断言人工审核提醒的开关/阈值判定（全过才返回 0）
ReplayTool manual-fit <offset-samples.jsonl> [日期]  # 跑一遍手动拟合：当天有效样本 → 要写入的偏移
```
