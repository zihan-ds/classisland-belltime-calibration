...
6. **防御**：整窗静默（麦克风未进音）直接作废；一次要改偏移超过「大误差复核阈值」（默认 ±3 s）时**拒写**，
   并**在界面上弹出人工复核提醒**（见下节）。

## 大误差人工复核提醒

一次要写入的偏移与当前偏移相差超过阈值时，插件拒绝写入并弹出一条提醒：边界时刻、当前偏移、
本次测得的所需偏移与差值，要求人工确认后再决定是否手动校准。

- 只在**真的拒写**时出现；**没有时间冷却**——同一个大误差每次都提醒；
- 由设置项**「人工审核提醒」**（默认开）控制：关掉后大误差仍然拒写，但不再弹提醒、只记日志
  （拒写是数据安全底线，提醒只是可选的打扰，两者分开）；
- 出现在 ClassIsland 的提醒通道里，可在**设置 → 提醒 → 铃声校时复核**单独关闭（也可调整提醒音/置顶等宿主偏好）；
- 设置页有**「发送测试提醒」**按钮，可随时确认提醒能正常弹出（该测试不受开关约束）；
- 提醒走宿主已有的提醒主机服务（公开实例 + 反射调用其 internal 发送方法），**不需要任何内核改动**。

> 被静默拦下的偏差，用户根本不知道要去看日志——所以这条路径必须有出口。

## 手动拟合（设置页「手动拟合并应用」）

按下后：读当天已落盘样本 → **只保留有效样本**（`Applied = true`，即真正写入过偏移的干净测量；
大误差拒写与死区带内的存量不计入）→ 用与运行时**完全相同**的估计器（近 6 小时中位数，
趋势外推仅在样本数/跨度/残差三门槛同时满足时启用）重算 → **立即写入「应用设置 → 时钟 → 时间偏移」**。
**无视学习模式与自动应用开关**（显式手动动作，按下即写）。

写入后会把当天样本重新装进运行时估计器，避免下一个边界用旧序列把手动结果顶回。
...
| 大误差复核阈值（秒） | 3 | 一次要改的偏移超过此值时**拒写**并弹出人工复核提醒（1~60 s，改完即生效）；它同时是「是否提醒」的判据，因此不接受把它设成极大值来关掉保护 |
| 人工审核提醒 | 开 | 关掉后大误差仍然拒写，但不再弹出界面提醒（只记日志）。拒写不受此开关影响 |
...
│  ├─ BellReviewNotifier.cs       # 大误差复核提醒：文案 + 宿主提醒通道（HostNotificationSender）
│  ├─ ManualFitService.cs         # 手动拟合：当天有效样本 → 所需偏移（设置页按钮背后的纯逻辑）
│  ├─ IOffsetApplier.cs           # 应用通道接口（唯一实现为内核设置通道）
│  └─ SettingsOffsetApplier.cs    # 反射读写内核设置项 TimeOffsetSeconds（唯一应用通道；课表平移已移除）
